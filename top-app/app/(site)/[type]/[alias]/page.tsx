...
export const generateStaticParams = async () => {
    ...
    for (const m of firstLevelMenu) {
        ...
        paths = paths.concat(menu.flatMap(s => s.pages.map(p => {
            return {
                type: m.route,
                alias: p.alias
            }
...
const Course = async ({params}: { params: { alias: string, type: string } }): Promise<JSX.Element> => {
    ...
    if (!page) notFound();
    return (
        <TopPageComponent firstCategory={firstCategory.id} page={page} products={products}/>
    );
};
